...
function loadCloudinaryConfig() {
  // Prefer explicit vars, else fall back to CLOUDINARY_URL
  const explicit = {
...
// Helper to create Cloudinary signature for direct uploads
...
// Admin: get a signature for client-side direct uploads to Cloudinary
...
// Admin: persist metadata for images uploaded to Cloudinary client-side
// Body: { items: Array<Partial<CloudinaryUploadResult>> }
router.post('/batch', auth, admin, async (req, res) => {
  try {
    ...
    // Optional: broadcast new images event
...
// Public: list images for user app
